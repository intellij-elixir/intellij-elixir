...
public class Parameter {
    ...
    private static Parameter putParameterized(@NotNull final Parameter parameter, final @NotNull Call ancestor) {
        ...
        if (form != null) {
            // A `defguard` is a CLAUSE like the rest, and runs at run-time, so its name is a FUNCTION_NAME. Only
            // `isMacro` still splits the two, which `Form` does not say.
            Type nameType = CallDefinitionClause.isMacro(ancestor) ? Type.MACRO_NAME : Type.FUNCTION_NAME;
